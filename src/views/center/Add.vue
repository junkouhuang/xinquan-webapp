<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>新增中心</el-breadcrumb-item>
    </el-breadcrumb>
...
<script>
import { addCenter } from '@/api/center'
...
export default {
  ...
  data() {
    ...
    const validateLatLng = (rule, value, callback) => {
      let latReg = /^[\-\+]?(0?\d{1,2}\.\d{1,9}|1[0-7]?\d{1}\.\d{1,9}|180\.0{1,9})$/
      let lngReg = /^[\-\+]?([0-8]?\d{1}\.\d{1,9}|90\.0{1,9})$/
      if (!this.form.latitude || !this.form.longitude) {
        return callback(new Error('请输入经纬度'))
      } else if (!lngReg.test(this.form.longitude) || !latReg.test(this.form.latitude)) {
        return callback(new Error('请输入正确的经纬度'))
      }
      callback()
    }
    return {
