  ...
  <div>
        ...
        <el-row :gutter="20">
          ...
          <el-col :xl="{span: 6}" :lg="{span: 8}" :sm="{span: 12}">
            <el-form-item label="类型" prop="type">
              <el-select placeholder="请选择部门类型" v-model="query.type" style="width: 100%" clearable>
                <el-option label="全部" value=""></el-option>
                <el-option
          ...
          <el-col :xl="{span: 6}" :lg="{span: 8}" :sm="{span: 12}">
            <el-button type="blue" v-has="'res_department_list_query'" @click="getList">查 询</el-button>
    ...
    <el-card shadow="never" class="mt20">
      <div slot="header" class="clearfix">
        <div class="pull-left">
          <el-button type="blue" v-has="'res_department_add'" @click="handleAdd">新增部门</el-button>
        </div>
      ...
      <el-table
        ...
        <el-table-column
          ...
          label="类型">
           <template slot-scope="scope">
            <span v-if="scope.row.departmentType" >机构</span>
            <span v-else >系统</span>
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column label="操作" width="80">
          <template slot-scope="scope">
            <el-tooltip class="item" v-has="'res_department_edit'" effect="dark" content="编辑" placement="top">
              <el-button class="reset-icon" @click="handleEdit(scope.row)" type="text" icon="el-icon-edit-outline"></el-button>
            </el-tooltip>
            <el-tooltip class="item" v-has="'res_department_delete'" effect="dark" content="删除" placement="top">
    ...
    <el-dialog
      ...
      <div slot="footer" style="text-align: center">
        ...
        <el-button v-if="this.editvisiable" type="blue" v-has="'res_department_edit'" @click="handleConfirm">确 定</el-button>
        <el-button v-else type="blue" v-has="'res_department_add'" @click="handleConfirm">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  data() {
    const validateName = (rule, value, callback) => {
      ...
      alert(this.selOrgId)
      formData.append('orgId', this.selOrgId == null? '' : this.selOrgId)
  ...
  methods: {
    ...
    typeList() {
      var params = {}
      this.axios.get('/common/department/types',{params}).then(res => {
        if (res.returnCode == 'ERR_0000') {
    ...
    getList() {
      ...
      this.axios.post('/authorizes/department/query', formData).then(res => {
        if (res.returnCode == 'ERR_0000') {
    ...
    handleEdit(row) {
      this.selOrgId = row.organizationId
    ...
    handleConfirm() {
      ...
      this.$refs['departmentForm'].validate(valid => {
        ...
        if (this.dialogType == 'add') {
          ...
          this.axios.post('/authorizes/department/create', formData).then(res => {
            if (res.returnCode == 'ERR_0000') {
              this.$message({
                message: '部门添加成功',
                type: 'success'
