  ...
  <div>
    <el-card shadow="never" class="mt20">
      <el-form :model="form" :rules="rules" ref="ruleForm" label-width="100px" >
        ...
        <el-form-item>
          <el-button type="blue" v-has="'res_mmc_feedback_edit'" @click="submitForm">保存提交</el-button>
          <el-button @click="$router.push({path: '/mmc/order'})">返回</el-button>
        </el-form-item>
      </el-form>
    </el-card>
  </div>
        ...
        if (valid) {
          request.edit(this.form).then(res => {
            if (res.returnCode == "ERR_0000") {
              ...
              this.$router.push({ path: "/mmc/order" });
            } else {
              this.$message({
                message: "反馈修改失败",
                type: "error"
