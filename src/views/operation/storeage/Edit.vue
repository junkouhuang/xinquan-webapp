  ...
  <div>
    <el-card shadow="never" class="mt20">
      <el-form :model="form" :rules="rules" ref="ruleForm" label-width="120px" >
        ...
        <br/><br/>
        <el-form-item>
          <el-button type="blue" v-has="'res_media_resource_save'" @click="submitForm">保存提交</el-button>
          <el-button @click="$router.push({path: '/operation/storeage'})">返回</el-button>
        </el-form-item>
      </el-form>
    </el-card>
  </div>
  ...
  methods: {
    ...
    queryRequest() {
      ...
      request.detail(this.$route.params.id).then(res => {
        if (res.returnCode == "ERR_0000") {
          that.form = res.data
          that.uploadFileName = res.data.storageName
        }
      });
    },
    //  覆盖默认上传行为
        ...
        if (valid) {
          request.edit(this.form).then(res => {
            if (res.returnCode == "ERR_0000") {
              ...
              this.$router.push({ path: "/operation/storeage" });
            } else {
              this.$message({
                message: "资源修改失败",
                type: "error"
