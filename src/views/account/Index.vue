  ...
  <div>
        ...
        <el-row :gutter="20">
          ...
          <el-col :xl="{span: 6}" :lg="{span: 8}" :sm="{span: 12}">
            <el-form-item label="账号类别" prop="type">
              <el-select placeholder="请选择账号类别" v-model="accountData.type" style="width: 100%" clearable>
                <el-option
          ...
          <el-col :xl="{span: 6}" :lg="{span: 8}" :sm="{span: 12}">
            <el-form-item label="账号类型" prop="category">
              <el-select placeholder="请选择账号类型" v-model="accountData.category" style="width: 100%" clearable>
                <el-option
          ...
          <el-col :xl="{span: 6}" :lg="{span: 8}" :sm="{span: 12}">
            <el-button type="blue" v-has="'res_acount_list_query'" @click="searchAccount">查 询</el-button>
    ...
    <el-card shadow="never" class="mt20">
      <div slot="header" class="clearfix">
        <div class="pull-left">
          <el-button type="blue" v-has="'res_account_add'" @click="$router.push({path: '/system/account/add'})">新增账号</el-button>
          <el-button type="danger" v-has="'res_account_batch_delete'" @click="handleDeleteSelected">删除账号</el-button>
        </div>
      ...
      <el-table @selection-change="changeSelection"
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <span v-else style="color:#bbb">鑫健康</span>
          </template>
        </el-table-column>
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <span v-else-if="scope.row.state == 2" class="label label-warning" @click="headleRelease(scope.row)">锁定</span>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-tooltip class="item" v-has="'res_account_reset'" effect="dark" content="重置密码" placement="top">
              <el-button class="reset-icon" @click="resetPwd(scope.row)" type="text" >
                <i class="iconfont">&#xe641;</i>
              </el-button>
            </el-tooltip>
            <el-tooltip class="item" v-has="'res_account_edit'" effect="dark" content="编辑" placement="top">
              <el-button class="reset-icon" @click="$router.push({ path: `/system/account/${scope.row.accountName}` })" type="text" >
                <i class="iconfont">&#xe71c;</i>
              </el-button>
            </el-tooltip>
            <el-tooltip class="item" v-has="'res_account_delete'" effect="dark" content="删除" placement="top">
              <el-button class="reset-icon" @click="handleDelete(scope.row)" type="text">
                <i class="iconfont">&#xe614;</i>
              </el-button>
            </el-tooltip>
            <el-tooltip class="item" v-has="'res_account_permission'" effect="dark" content="分配角色" placement="top">
    ...
    <el-dialog title="选择角色" :visible.sync="dialogTableVisible">
      ...
      <div slot="footer" style="text-align:center">
        <el-button type="blue" v-has="'res_account_permission'" @click="roleSubmit">确定</el-button>
...
import { getAccountRoles, createRole } from '@/api/system/roles'
export default {
  data() {
    return {
    ...
    handleAllot(row) {
      account.detailRequest({
        ...
        if (res.returnCode == 'ERR_0000') {
          ...
          getAccountRoles(this.accountDetail).then(res => {
            if (res.returnCode == 'ERR_0000') {
