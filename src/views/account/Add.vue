    ...
    <el-card shadow="never" class="mt20">
      <el-form :model="ruleForm" :rules="rules" ref="accountForm" label-width="100px" style="max-width: 600px">
        ...
        <el-form-item label="账号类别" prop="type">
          ...
          <el-alert v-if="orgAccRulesVisible"
             title="同一机构只允许创建一个主管理账号"
             type="warning" style="margin-top:5px;padding:0px;">
          </el-alert>
        </el-form-item>
        ...
        <el-form-item>
          <el-button type="blue" v-has="'res_account_add'" @click="submit">保存提交</el-button>
...
export default {
  data() {
    ...
    return {
      ...
      passVisible1: false,
      passVisible2: false,
      orgAccRulesVisible: false,
    };
  },
  watch: {
    'ruleForm.type': function(val) {
      if (val === 0) {
        this.orgAccRulesVisible = false
        this.ruleForm.organizationId = ''
      }else {
        this.orgAccRulesVisible = true
      }
    }
  },
  created(){
  ...
  methods: {
    selectChange() {
      if (this.ruleForm.type == 1) {
        account.getOrgDepartmentData(this.ruleForm.organizationId).then(res => {
      ...
      this.$refs['accountForm'].validate(valid => {
        if (valid) {
          account.addRequest(this.ruleForm).then(res =>{
            ...
            }else if (res.returnCode == 'ERR_0009'){
              this.$message({
                message: '同一机构仅支持一个主管理账号,当前机构主账号已注册，请勿重复注册',
                type: 'error'
              })
            }else {
               this.$message({
