  ...
  <div>
    <el-card shadow="never" class="mt20">
      <el-form :model="ruleForm" :rules="rules" ref="accountForm" label-width="85px" style="max-width: 600px">
        ...
        <el-form-item>
          <el-button type="blue" v-has="'res_account_edit'" @click="submit">保存提交</el-button>
...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        ...
        realName: [
          { required: true, message: "请输入账号使用人的真实姓名", trigger: "blur" }
        ],
        state: [
  ...
  methods: {
    selectChange() {
      if (this.ruleForm.type == 1 && this.category == 1 && this.changeVisiable) {
    ...
    detailRequest() {
      account.detailRequest({'accountName':this.$route.params.id}).then(res => {
        if(res.returnCode == 'ERR_0000'){
          ...
          if (res.data.type == 1 && res.data.category == 1) {
            this.changeVisiable = true
